...
reelforge는 한국어 나레이션 중심의 가로, 세로, 정사각 영상 생성을 목표로 합니다. 렌더 엔진은 `hyperframes@0.7.26`으로 정확히 고정하고, 편집은 HTML을 직접 고치는 대신 `scene_specs` 같은 계약 파일을 수정한 뒤 재컴파일하는 단일 경로를 사용합니다.
